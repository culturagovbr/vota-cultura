...
class Upload extends AbstractService
{
    ...
    {
        $arquivoDecodificador = base64_decode($stringCodificada);
        $mime_type = finfo_buffer(
            finfo_open(),
            $arquivoDecodificador,
            FILEINFO_MIME_TYPE
        );
        $extension = mime2ext($mime_type);
        $file = uniqid() . '.' . $extension;

        if (!file_exists($diretorioArmazenamento)) {
            mkdir(
                $diretorioArmazenamento,
                0755,
                true
        ...
        $caminhoCompletoArquivo = $diretorioArmazenamento . $file;

        try {
            file_put_contents($caminhoCompletoArquivo, $arquivoDecodificador);
        } catch (\Exception $e) {
